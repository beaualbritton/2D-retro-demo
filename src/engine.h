...
using std::vector, std::unique_ptr, std::make_unique, glm::ortho, glm::mat4,
    glm::vec3, glm::vec4;
...
private:
   ...
   * Holy mother of god...
   *
   * Trust me. ALL of these bolleans are necessary for the simple battle screen
   * you see in this demo. These are FLAGS. nothing more. trying to track game
   * states is not fun.
   *
  ...
  bool battleTextDisplayed = false;
  bool infoTextDisplayed = false;
  bool enemyTextDisplayed = false;

  bool playerAttacking = false;
  bool playerDefending = false;
  bool playerViewing = false;
...
public:
  /*
   * these are for platforming physics calculations in engine.cpp, reason these
   * are set as public are due to const nature, these are *physical* constants.
   */
  ...
  /// The projection matrix transforms coordinates in the camera space into
  ...
  // 1st quadrant
  //        mat4 PROJECTION = ortho(0.0f, static_cast<float>(width), 0.0f,
