...
entity makeNewCharacter()
{
  ...
  /*
   *Name, description, health and experience inputs. All pretty self
   *explanatory. Uses input.h methods to validate input properly
   */
  ...
  entity entityToReturn =
      entity(health, experience, name, description, alignment);
 ...
 * if there is data that can be found, continue and see if it is empty or not.
 * If the data cannot be found "Error" is printed with cout.
 *
 * Then, if data can be found, it reads it and sees if its empty, or not. If so
 * it either prompts the use to continue or make a new character. If the player
 * continues, it reads save data and loads an appropriate entity matching the
 * input from playerinfo.csv
 *
 * Or, all else, a new character is created as a failproof. (couldn't read data
 * or misinput)
 *
 */
void player::makePlayer()
{
  ...
  if (fileIn)
  {
    ...
    else
    {
      // Save data found
      cout << endl
	   << "Save data found. Would you like to (c)ontinue or make a (n)ew "
	      "character?"
	   << endl
	   << endl;
	...
	int alignment;
	char comma = ',';
	// Reading values in from fileIn and assigning them to variables via >>
	// operator
...
void player::save()
{
  ...
  ofstream fileOut;
  fileOut.open("entity-data/playerinfo.csv");

  // If file can be opened
  if (fileOut)
  {
    // Read these values to the output file:
    // name,description,health,experience,alignment
 ...
 */
void player::setPlayerEntity(entity &pEntity) { this->playerEntity = pEntity; }
// Returns a refrence of the current playerEntity
entity &player::getPlayerEntity() { return this->playerEntity; }
